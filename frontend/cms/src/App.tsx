...
export default function App() {
  ...
  const [authToken, setAuthToken] = useState<string | null>(() => {
    try {
      return localStorage.getItem('chiro_token');
    } catch {
      return null;
    }
  });
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => !!getStoredToken());
  const [isLoginModalOpen, setIsLoginModalOpen] = useState<boolean>(() => !getStoredToken());
  ...
  const handleLoginSuccess = (user: CMSUser, token: string) => {
    setCurrentUser(user);
    setAuthToken(token);
    ...
    try {
      ...
      localStorage.setItem('chiro_token', token);
    } catch (e) {
  ...
  const handleLogout = () => {
    ...
    try {
      ...
      localStorage.removeItem('chiro_token');
    } catch (e) {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      const [docRes, roleRes] = await Promise.all([
        fetch('/api/doctors', { headers: authHeaders(false) }),
        fetch('/api/roles', { headers: authHeaders(false) })
      ]);
  ...
  useEffect(() => {
    ...
    try {
      const streamUrl = authToken ? `/api/realtime/stream?token=${encodeURIComponent(authToken)}` : '/api/realtime/stream';
